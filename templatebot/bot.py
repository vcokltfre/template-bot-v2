    ...
    def load_initial_cogs(self, *cogs):
        ...
        for cog in cogs:
            try:
                ...
                self.logger.error_s(f"Cog {cog} experienced an error during loading: {e}")
                failed += 1

        self.logger.info_s(f"Cog loading complete! (Total: {success + failed} | Loaded: {success} | Failed: {failed})")
